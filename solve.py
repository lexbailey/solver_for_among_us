...
class GameSolver:
    ...
    def __init__(self, n_players, n_impostors):
        ...
        self.alive, self.murdered, self.impostor, self.ejected = (
            [z3.Bool('%s-%d' % (varname, player_id)) for player_id in range(max_players)]
            for varname in [
                'player-is-alive'
                ,'player-was-murdered'
                ,'player-is-impostor'
    ...
    def basic_constraints(self):
        ...
        return [
            ...
            # People that were murdered or ejected are not alive
            ,*[
                z3.Implies(z3.Or(self.murdered[i], self.ejected[i]), z3.Not(self.alive[i]))
                for i in range(max_players)
            ]
            # People that dont exist are not alive
            ,*[
                z3.Not(self.alive[i])
                for i in range(max_players) if i > self.n_players-1
            ]
            # People that don't exist are not impostors
            ,*[
                z3.Not(self.impostor[i])
                for i in range(max_players) if i > self.n_players-1
            ]
        ]
    ...
    def combine_solutions(self, models):
        ...
        counts = [0 for _ in range(max_players)]
        for model in models:
            for i in range(max_players):
